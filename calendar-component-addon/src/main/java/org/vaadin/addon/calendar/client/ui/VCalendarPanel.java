...
public class VCalendarPanel extends FocusableFlexTable implements KeyDownHandler, KeyPressHandler, MouseOutHandler,
        MouseDownHandler, MouseUpHandler, BlurHandler, FocusHandler, SubPartAware {

    private final Map<DateTimeResolution, Integer> fieldResolutionMapping = new HashMap<>();
    ...
    public VCalendarPanel() {
        ...
        fieldResolutionMapping.put(DateTimeResolution.SECOND, 13);
        fieldResolutionMapping.put(DateTimeResolution.MINUTE, 12);
        fieldResolutionMapping.put(DateTimeResolution.HOUR, 11);
        fieldResolutionMapping.put(DateTimeResolution.DAY, 5);
        fieldResolutionMapping.put(DateTimeResolution.MONTH, 2);
        fieldResolutionMapping.put(DateTimeResolution.YEAR, 1);
    ...
    private void focusDay(Date date) {
        ...
        if (fieldResolutionMapping.get(resolution) > fieldResolutionMapping.get(DateTimeResolution.MONTH)) {
            if (focusedDay != null) {
    ...
    public void setShowISOWeekNumbers(boolean showISOWeekNumbers) {
        ...
        if (initialRenderDone
                && fieldResolutionMapping.get(getResolution()) >= fieldResolutionMapping.get(DateTimeResolution.DAY)) {
    ...
    private void buildCalendarBody() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (fieldResolutionMapping.get(getResolution()) > fieldResolutionMapping.get(DateTimeResolution.MONTH)) {
    ...
    private boolean isTimeSelectorNeeded() {
        return fieldResolutionMapping.get(getResolution()) > fieldResolutionMapping.get(DateTimeResolution.DAY);
    }
    ...
    public void renderCalendar(boolean updateDate) {
        ...
        if (updateDate
                && fieldResolutionMapping.get(getResolution()) <= fieldResolutionMapping.get(DateTimeResolution.MONTH)
                && focusChangeListener != null) {
            focusChangeListener.focusChanged(new Date(focusedDate.getTime()));
        }

        final boolean needsMonth = fieldResolutionMapping.get(getResolution()) > fieldResolutionMapping.get(DateTimeResolution.YEAR);
        boolean needsBody = fieldResolutionMapping.get(getResolution()) >= fieldResolutionMapping.get(DateTimeResolution.DAY);
    ...
    public void setDate(Date currentDate) {
        ...
        if (value == null || currentDateWasAdjusted) {
            ...
            if (rangeStart != null || rangeEnd != null) {
                ...
                // a day
                // resolution
                if (fieldResolutionMapping.get(getResolution()) >= fieldResolutionMapping.get(DateTimeResolution.DAY)) {
                    value = null;
                }
            } else {
    ...
    public class VTime extends FlowPanel implements ChangeHandler {
        ...
        private ListBox createListBox() {
            ...
            lb.setStyleName("v-select"); // FIXME CSS-Style hardcoded
        ...
        private void buildTime() {
            ...
            if (fieldResolutionMapping.get(getResolution()) >= fieldResolutionMapping.get(DateTimeResolution.MINUTE)) {
                mins = createListBox();
                for (int i = 0; i < 60; i++) {
            ...
            if (fieldResolutionMapping.get(getResolution()) >= fieldResolutionMapping.get(DateTimeResolution.SECOND)) {
                sec = createListBox();
                for (int i = 0; i < 60; i++) {
